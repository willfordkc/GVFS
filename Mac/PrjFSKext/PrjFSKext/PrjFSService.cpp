    ...
    case UserClientType_Provider:
        {
            ...
            {
                const pid_t userProcessPID = proc_selfpid();
                ActiveProvider* provider = ActiveProvider_RegisterUserClient(provider_client, userProcessPID);
                if (nullptr != provider)
                {
                    provider_client->provider = provider;
                    *handler = provider_client;
                    result = kIOReturnSuccess;
                }
                else
                {
                    StopDetachReleaseUserClient(this, provider_client);
