...
struct VirtualizationRootXAttrData
{
    XattrHeader header;
};

struct FileXAttrData
{
    XattrHeader header;
...
static bool ConnectToDriverService();
...
// Constants
static const char* VirtualizationRootXAttrName = "io.gvfs.xattr.virtualizationroot";
static const char* FileXAttrName = "io.gvfs.xattr.file";
    ...
    _In_    unsigned int                            poolThreadCount)
{
    ...
    if (!DataQueue_Init(&dataQueue, s_kernelServiceConnection, ProviderPortType_MessageQueue, ProviderMemoryType_MessageQueue, s_messageQueueDispatchQueue))
    {
    ...
    _In_    const char*                             virtualizationRootFullPath)
{
    ...
    VirtualizationRootXAttrData rootXattrData = {};
    if (!InitializeEmptyPlaceholder(
            virtualizationRootFullPath,
            &rootXattrData,
            VirtualizationRootXAttrName))
    {
...
static PrjFS_Result HandleHydrateFileRequest(const MessageHeader* request, const char* path)
{
    ...
    FileXAttrData xattrData = {};
    if (!GetXAttr(fullPath, FileXAttrName, sizeof(FileXAttrData), &xattrData))
    {
        return PrjFS_Result_EIOError;
    }
...
static bool IsVirtualizationRoot(const char* path)
{
    VirtualizationRootXAttrData data = {};
    if (GetXAttr(path, VirtualizationRootXAttrName, sizeof(VirtualizationRootXAttrData), &data))
    {
        return true;
    }
    ...
    dispatch_queue_t eventHandlingQueue)
{
    ...
    IOConnectSetNotificationPort(connection, clientPortType, outQueue->notificationPort, 0);
...
static bool ConnectToDriverService()
{
    ...
    {
        cerr << "PrjFS kernel service does not advertise valid kext version property\n";
        goto CleanupAndFail;
    }
    ...
    if (kCFCompareEqualTo != CFStringCompare(kextVersionString, CFSTR(PrjFSKextVersion), 0 /* options: case sensitive */))
    {
        ...
        cerr << "PrjFS kernel service interface version mismatch. Kernel: " << kextVersion << ", this library expects: " << PrjFSKextVersion << endl;
        goto CleanupAndFail;
    }
    else
    {
        ...
        kern_return_t result = IOServiceOpen(prjfsService, mach_task_self(), UserClientType_Provider, &connection);
        IOObjectRelease(prjfsService);
        if (result != kIOReturnSuccess || connection == IO_OBJECT_NULL)
        {
            std::cerr << "Failed to open connection to kernel service" << std::endl;
            return false;
        }
        s_kernelServiceConnection = connection;
    }
