...
namespace GVFS.Mount
{
    public class InProcessMount
    {
        ...
        private void HandleLockRequest(string messageBody, NamedPipeServer.Connection connection)
        {
            ...
            else
            {
                ...
                if (externalHolder == null &&
                    this.fileSystemCallbacks.IsReadyForExternalAcquireLockRequests(requester, out denyMessage))
                {
        ...
        private void MountAndStartWorkingDirectoryCallbacks(CacheServerInfo cache)
        {
            ...
            WindowsFileSystemVirtualizer windowsVirtualizer = this.CreateOrReportAndExit(() => new WindowsFileSystemVirtualizer(this.context, this.gitObjects), "Failed to create src folder virtualizer");
            this.fileSystemCallbacks = this.CreateOrReportAndExit(() => new FileSystemCallbacks(this.context, this.gitObjects, RepoMetadata.Instance, windowsVirtualizer), "Failed to create src folder callback listener");
