...
";
        ...
        {
            TestAddingPath("dir\\somefile.txt");
        }

        [TestCase]
        public void DirectorySeparatorsNormalized()
        {
            TestAddingPath("dir\\dir2\\dir3\\somefile.txt");
        }

        [TestCase]
        public void BeginningDirectorySeparatorRemoved()
        {
            const string FilePath = "dir\\somefile.txt";
            TestAddingPath(pathToAdd: "\\" + FilePath, pathInList: FilePath);
        }

        [TestCase]
        public void DirectorySeparatorAddedForFolder()
        {
            TestAddingPath(pathToAdd: "dir\\subdir", pathInList: "dir\\subdir\\", isFolder: true);
        }
        ...
        {
            return path.Replace(GVFSConstants.PathSeparator, GVFSConstants.GitPathSeparator);
        }

        private static string ToPathSeparators(string path)
        {
            return path.Replace(GVFSConstants.GitPathSeparator, GVFSConstants.PathSeparator);
        }
